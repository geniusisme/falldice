    ...
    I::Item: Iterator,
{
    fn new(iter: I, f: F) -> Self {
        CartesianProductCollections { items: Items::InIterator(iter), f }
    }
}
    ...
    fn next(&mut self) -> Option<R> {
        ...
        self.items = match old_items {
            Items::InIterator(iter) => {
                ...
                let current_items = current_iters.iter_mut()
        ...
        match self.items {
            Items::Evaluated(ref items) => {
                Some((self.f)(items.current_items.iter()))
            },
            Items::Empty => None,
            _ => unreachable!(),
        }
    ...
    assert_eq!(
        (0..3)
            ...
            .cartesian_collections(|iter| {
                iter.fold(0, |pr, next| pr * 10 + *next)
            })
            .collect::<Vec<_>>(),
